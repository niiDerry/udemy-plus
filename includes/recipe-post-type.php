...
function up_recipe_post_type(){
  $labels = array(
    ...
    'taxonomies'         => ['category', 'post_tag'], // Adds categories and tags support
	);
  ...
  register_taxonomy( 'cuisine', 'recipe', [
	...
	// This function needs to be called after the taxonomy is registered
	...
	// this is an alternative to using add_post_meta() and update_post_meta() - see includes/admin/save-recipe.php for example
	register_post_meta('recipe', 'recipe_rating', [
		'type' => 'number',
		'description' => __('A rating for the recipe from 0 to 5', 'udemy-plus'),
		'single' => true,
		'default' => 0,
		'show_in_rest' => true,
	]);
